...
class ImgCompression(object):
    ...
    def compress(
        ...
        if U.ndim ==2:
            ...
            S_compressed = S[:, :k]
            V_compressed = V[:, :k,:]
        # print(U_compressed.shape, S_compressed.shape, V_compressed.shape)
    ...
    def rebuild_svd(
        ...
        """
        if U_compressed.ndim == 2:  # Black and white image
            X_rebuild = np.matmul(U_compressed, np.matmul(np.diag(S_compressed), V_compressed))
        else:  # Color image (RGB)
            num_channels = U_compressed.shape[0]
            X_rebuild = np.zeros((num_channels, U_compressed.shape[1], V_compressed.shape[2]))
            for channel in range(num_channels):
                X_rebuild[channel, :, :] = np.matmul(U_compressed[channel, :, :], np.matmul(np.diag(S_compressed[channel, :]), V_compressed[channel, :, :]))
        # print(X_rebuild.shape)
        return X_rebuild
    ...
    def compression_ratio(self, X: np.ndarray, k: int) -> float:  # [4pts]
        ...
        """
        num_original_values = X.size 
        if X.ndim == 2:
            num_compressed_values = (k * (X.shape[0]) + k + (k * X.shape[1]))
        else:
            num_compressed_values = (k * (X.shape[1]) + k + (k * X.shape[2]))*3

        compression_ratio = num_compressed_values / num_original_values
        # print("k: ", k)
        # print(num_compressed_values, num_original_values, compression_ratio)
    ...
    def recovered_variance_proportion(self, S: np.ndarray, k: int) -> float:  # [4pts]
        ...
        if S.ndim == 1:
            ...
            retained_variance = np.sum(S[:, :k]**2, axis=1)

        # print(total_variance, retained_variance)
        proportion_recovered_variance = retained_variance / total_variance
        # print("ans: ", proportion_recovered_variance)
        return proportion_recovered_variance
